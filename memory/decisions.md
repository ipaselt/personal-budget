...
## 2026-07-05 — Merchant learning stays GLOBAL, including archived years (reviewed, kept)
The pre-merge ultra review flagged that recategorizing a merchant applies the learned rule across *all* years
via `setUserCatByPattern` (`UPDATE … WHERE UPPER(name)=?`, no year filter), so it mutates archived (read-only)
years' `user_category` too — a tension with the "archived views are read-only" guarantee. **Owner chose to
keep it global** (consistent cross-year merchant categorization is the point of the feature; see the
...
deleted month (until the next import re-ran `upsertAccount`). Fixed by adding `recomputeAccountBalances` (sets
each surviving account's balance/as_of from its latest-dated balance-carrying txn, `rowid DESC` tiebreak to
match import insertion order) and calling it in `clear_month`. Verified: import Jan(500)+Feb(900) → clear Feb
→ balance 500 → clear Jan → account removed, balance 0.
