...
## How it works
- **Import (multi-format, two-step preview→confirm):** the file picker sends **raw bytes** to `POST /api/import_preview` (parse, **no save**) → a review modal shows the detected format + columns, a sample of parsed rows (red/green = out/in), and new-vs-duplicate counts; confirming re-sends the bytes to `POST /api/import` (save). `analyzeImport(buf, name)` **sniffs the bytes** and dispatches: **CSV**/**Excel** (`.xls`/`.xlsx` via SheetJS) both reduce to rows → shared `rowsToRecords()` column-detection; **QFX/OFX** → `ofxToRecords()` (parses `<STMTTRN>` blocks, dedups on the bank's `FITID`, balance from `<LEDGERBAL>`). Column auto-detection uses header-name synonyms (Date/Post Date, Description/Memo/Payee/Merchant/Details, Debit/Withdrawal + Credit/Deposit *or* Amount, Balance, Status, Account) and parses ISO / US `M/D/Y` / textual (`Jan 5, 2026`) dates — **generic across banks**, not tuned to one. Ardent Credit Union is just the default shape. (Note: same statement imported as both QFX and CSV/Excel won't cross-dedup — FITID vs content-hash — but users pick one format.)
...
- 📄 **Multi-format import: QFX/OFX + Excel (XLS/XLSX), added 2026-07-07** — since not every bank exports CSV. `analyzeImport()` sniffs the uploaded bytes and dispatches: CSV/Excel → rows → shared `rowsToRecords()`; QFX/OFX → `ofxToRecords()` (FITID dedup, LEDGERBAL). Frontend now uploads raw bytes (`arrayBuffer`); endpoints use `express.raw`. Excel via **SheetJS** — installed the **patched CDN build `xlsx@0.20.3`** (npm's 0.18.5 has prototype-pollution + ReDoS advisories; CDN URL dep in package.json). Verified all 3 formats parse + import + dedup on an isolated temp DB; real data untouched. See `memory/decisions.md`.
